...
export class FlowCreator {
	constructor(manager: FlowManager, uiManager: UIManager, mainScene: MainScene, soundManager: SoundManager) {
		...
		manager.addFlow(new Flow(FlowEventName.Move, [
			soundStep,
			new TransStep()
		]));
		manager.addFlow(new Flow(FlowEventName.Rotate, [
			soundStep,
			new TransStep()
		]));

		manager.addFlow(new Flow(FlowEventName.AddScore, [
			soundStep,
			new UIStep(uiManager)
		]));
